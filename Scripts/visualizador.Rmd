...
```{r, echo=false}
# WORD CLOUD FUNCTION 
WordsToRemove <- Word_freq %>% dplyr::select(Var1) %>% slice(1:20)
...
```{r}
tfid1<-read.table("Results/Entities/cluster/matched_PCA_perp30_10_tfidf.txt", header = F, sep = '\t', quote = '', comment.char = "")
colnames(tfid1)<-c("clusters", "entity","word","frec","cluster_app","tfidf")
tfid1$term<-"m_term"

tfid2<-read.table("Results/Entities/cluster/prefered_PCA_perp30_10_tfidf.txt", header = F, sep = '\t', quote = '', comment.char = "")
...
files <- list.files(path="Results/Entities/pmid/", pattern="matched_", full.names=TRUE, recursive=FALSE)
...
files <- list.files(path="Results/Entities/pmid/", pattern="prefered_", full.names=TRUE, recursive=FALSE)
for(file in 1:length(files)){
  t <- read.table(files[[file]], header = F, sep = '\t', quote = '')
    prefered_pmids <- rbind(prefered_pmids,t)
}
...
u <- shinyUI(fluidPage(
  fluidRow(titlePanel("Article collection visualizer: E. Coli"), 
    ...
    column(8, box(span("Data sheet", style = "font-weight: bold; font-size: 18px")), htmlOutput(outputId = "infoPMID")) 
    ))),
...
s <- shinyServer(function(input, output) {
  ...
  observeEvent(input$matrix_click, {
    ...
    clst <- dataframe[xvals & yvals, 14] #corresponde al cluster
...
s <- shinyServer(function(input, output) {
  ...
  observeEvent(input$matrix_click, {
    ...
    xvals <- dataframe$dim1 %in% input$matrix_click$x
    yvals <- dataframe$dim2 %in% input$matrix_click$y
    clst <- dataframe[xvals & yvals, 14] #la col 10 de dataframe corresponde al cluster
  ...
  output$plotSpecificCluster <- renderPlot(
    if (input$tplot == "stack"){
      ...
      output$wordcloud2 <- renderWordcloud2({
       ...
       output$tablePMID <- renderTable({
          n_pmid<-read.table(paste0("Results/Entities/pmid/",art,'_frec.txt'), header = F, sep = '\t', quote = '')
